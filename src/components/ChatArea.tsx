...
export function ChatArea({
  ...
  useEffect(() => {
    ...
    const roomHandler = (message: any) => {
      if (
        selectedFriend?.type === "room" &&
        Number(message.roomId) === Number(selectedFriend.originalId)
      ) {
        setMessages((prev) => {
          if (prev.some((m) => m.id === message.id)) return prev;
          return [...prev, message];
  ...
  const handleSend = () => {
    ...
    const fId = selectedFriend.originalId
      ? parseInt(selectedFriend.originalId)
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col bg-chat-surface/60 text-foreground backdrop-blur-sm">
      ...
      <div
        className="min-h-0 flex-1 overflow-y-auto p-3 space-y-4 sm:p-4"
        ref={scrollRef}
      >
          ...
          return (
            ...
            >
              <Avatar className="mt-1 h-7 w-7 shrink-0 sm:h-8 sm:w-8">
                {msg.sender?.avatarUrl || msg.user?.avatarUrl ? (
                  <img
                    src={msg.sender?.avatarUrl || msg.user?.avatarUrl}
                    alt={senderName}
                    className="h-full w-full object-cover"
                  />
                ) : (
                  <AvatarFallback
                    className={`text-[10px] text-white sm:text-xs ${
